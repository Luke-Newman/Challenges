...
void BST<T>::getVisibleNodesPredecessor() const {
  ...
  for (int i : nodes_list) std::cout << i << " ";

  printf("\n");
}

template <class T>
void BST<T>::_markVisibleNodesPredecessor(const BST::spNODE &p,
...
template <class T>
void BST<T>::_markVisibleNodesParent(const BST::spNODE &p,
                                     std::vector<int> &nodes,
                                     int parent) const {
  if (p->data > parent) nodes.push_back(p->data);
...
void BST<T>::getMaxHeight() {
  ...
  while (!s.empty()) {
    ...
    /* if both leaves are null we have reached the end of a branch */
    ...
    if (current->left) {
      s.push(current->left.get());
    }
    if (current->right) {
      s.push(current->right.get());
    }
    current_depth++;
  }
  std::cout << "\nMax height: " << max_depth - 1 << '\n';
}
